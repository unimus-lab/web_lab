<template>
  ...
  <!--- Tambah alat baru --->
  <OffCanvas
    offcanvas_title="Tambah Bahan Padat"
  ...
  >
    <form @submit.prevent="addDataAlat">
      ...
      />
      <InputNumber
        label_name="Jumlah Bahan (gram)"
        placeholder_text="Jumlah Bahan (gram)"
  ...
  <!--- Pengadaan alat --->
  <OffCanvas
    offcanvas_title="Pengadaan Bahan Padat"
  ...
  <!--- lapor alat rusak --->
  <OffCanvas
    offcanvas_title="Lapor Bahan Padat Rusak"
...
const deleteDataAlat = async (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      deleteDoc(doc(db, "DaftarBahanPadat", id))
        .then(() => {
          DaftarBahan.data = [];
          getDataBahan();
        })
...
const addPengadaanAlat = async () => {
  await updateDoc(doc(db, "DaftarBahanPadat", PengadaanBahan.id), {
...
const addAlatRusak = async () => {
  await updateDoc(doc(db, "DaftarBahanPadat", LaporBahan.id), {
